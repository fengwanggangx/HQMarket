...
int main()
{
	...
	{
		std::cerr << "Failed to enable libevent thread support\n";
		return 1;
	}
	...
	if (!service.Initialize(9901, pszToken, root))
	{
	...
	http.RegisterHandler(net::HttpMethod::GET, "/v1/quotes",
						 [&service](const net::CHttpRequest& request)
						 {
							 ...
							 return Response(instrument.empty() ? 400 : 200,
											 instrument.empty() ? "{\"error\":\"instrument is required\"}"
																: service.QuoteJson(instrument),
											 "application/json; charset=utf-8");
						 });
	http.RegisterHandler(net::HttpMethod::GET, "/v1/bars",
						 [&service](const net::CHttpRequest& request)
						 {
							 std::string instrument = request.GetQuery("instrument");
							 if (instrument.empty())
							 ...
							 return Response(200, service.BarsJson(instrument, market::Channel::bar_1d, 0, INT64_MAX),
											 "application/json; charset=utf-8");
						 });
	if (http.Initialize() != 0)
	{
		service.Stop();
		tcpThread.join();
		return 4;
