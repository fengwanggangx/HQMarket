...
std::vector<market::CChannelInfo> CSubscriptionMgr::Subscribe(net::_TyConnectionId id, const std::vector<market::CChannelInfo>& channels)
{
	std::vector<market::CChannelInfo> adds;
...
std::vector<market::CChannelInfo> CSubscriptionMgr::Unsubscribe(net::_TyConnectionId id, const std::vector<market::CChannelInfo>& channels)
{
	std::vector<market::CChannelInfo> removed;
...
std::vector<market::CChannelInfo> CSubscriptionMgr::RemoveClient(net::_TyConnectionId id)
{
	std::vector<market::CChannelInfo> removed;
...
std::size_t CSubscriptionMgr::GetSubscriptionCount(const market::CChannelInfo& info) const
{
	std::string key = info.String();
	if (key.empty())
	{
...
bool CSubscriptionMgr::IsSubscribed(net::_TyConnectionId id, const market::CChannelInfo& info) const
{
	std::shared_lock<std::shared_mutex> lock(m_mtx_info);
	const auto mIter = m_id_infos.find(id);
